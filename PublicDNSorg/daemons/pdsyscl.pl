...
# $Source: /home/xubuntu/berlios_backup/github/tmp-cvs/pdnso/Repository/PublicDNSorg/daemons/pdsyscl.pl,v $
# $Revision: 1.1 $
...
pdsyscl.pl - Public-DNS.org client script to control pdsysd.pl daemons.
...
{
    print "usage: pdsyscl.pl SET DOMAIN ID CMD\n";
    print "       SET   - reload, add, update, remove\n";
    print "       DOMAIN- doamain that you want to slace\n";
    print "       ID    - ZonedID\n";
...
#open(OPEN,"testdomain.net");
#local $/;
#@stuff = <OPEN>;
#close(OPEN);
my @stuff = ($ARGV[0],$ARGV[1],$ARGV[2],<OPEN>,$ARGV[3]);
my $data = encrypt(@stuff);
print $sock $data;
#print $sock "pimp pimp";
close($sock);
